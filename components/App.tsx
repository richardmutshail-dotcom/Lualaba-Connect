...
import { 
  ...
  Loader2, Sparkles, Zap, Brain, Mic, Navigation, CloudRain, Cloud, CloudLightning, Moon, Volume2, VolumeX, Pause
} from 'lucide-react';
...
const FeedPostCard: React.FC<{ 
  ...
  return (
    <div className="min-w-[85%] snap-center bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col h-full">
        {/* Header */}
        ...
        {post.video ? (
          <div 
            className="rounded-xl overflow-hidden mb-3 border border-gray-100 dark:border-gray-700 h-40 shrink-0 relative group cursor-pointer"
            onClick={togglePlay}
          >
              <video 
                ref={videoRef}
              ...
              {!isPlaying && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-[1px]">
                    <div className="bg-white/20 text-white p-3 rounded-full backdrop-blur-md border border-white/30">
              ...
              <div className="absolute top-2 right-2 flex gap-2">
                 <button 
                    onClick={toggleMute}
                    className="bg-black/50 text-white p-1.5 rounded-full backdrop-blur-md hover:bg-black/70 transition-colors"
                 >
                    {isMuted ? <VolumeX size={14} /> : <Volume2 size={14} />}
                 </button>
              </div>
              <div className="absolute bottom-2 left-2 bg-black/50 text-white text-[10px] px-2 py-0.5 rounded font-bold backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity">
...
const App: React.FC = () => {
  ...
  const [homeFeed, setHomeFeed] = useState<{
  ...
  }[]>([
    {
      ...
      video: 'https://videos.pexels.com/video-files/2040062/2040062-sd_640_360_30fps.mp4',
      likes: 124,
      comments: 45,
      isVerified: true,
      isLiked: false
    },
    {
      ...
      video: 'https://videos.pexels.com/video-files/4255569/4255569-sd_640_360_25fps.mp4',
      likes: 56,
      comments: 12,
      isVerified: false,
      isLiked: false
    },
    {
      ...
      video: 'https://videos.pexels.com/video-files/854964/854964-sd_640_360_25fps.mp4',
      likes: 230,
      comments: 89,
      isVerified: true,
      isLiked: false
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case AppTab.HEALTH: return <HealthModule onBack={() => setActiveTab(AppTab.PROFILE)} setTabBarVisible={setIsTabBarVisible} />;
                      ...
                      <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar snap-x snap-mandatory -mx-6 px-6">
                         {homeFeed.map((post) => (
                            <FeedPostCard 
                                key={post.id} 
                                post={post} 
                                onLike={handleHomeLike} 
                                onComment={(id) => setActiveCommentId(id)}
                                onShare={handleHomeShare}
                            />
                         ))}
                      </div>
                   </div>
